...
`;
fileContent = fileContent.replace('const COLORS =', mockDataStr + 'const COLORS =');

const originalRenderLines = fileContent.split('\n');

// The ACTUAL return starts around line 232. We'll find it accurately.
const returnIndex = originalRenderLines.findIndex(l => l === '  return (');
...
let pieChartBlock = fileContent.substring(pieChartStart, pieChartEnd).trim();
...
// DO NOT remove any </div> from pieChartBlock because it's exactly one card.

let pendingStart = fileContent.indexOf('{/* Pending Fund Requests */}');
let pendingEnd = fileContent.indexOf('{/* Recent Transactions Ledger */}');
let pendingBlock = fileContent.substring(pendingStart, pendingEnd).trim();
...
// remove ONLY the last </div> (which closes the grid)
pendingBlock = pendingBlock.replace(/<\/div>\s*$/, '');

let txStart = fileContent.indexOf('{/* Recent Transactions Ledger */}');
let transactionsBlock = fileContent.substring(txStart).trim();
// transactionsBlock ends with:
//       </div>
//
//     </div>
//   );
// }
// We want to remove the `</div>\n  );\n}` which is the main wrapper div and the component closing.
transactionsBlock = transactionsBlock.replace(/<\/div>\s*\);\s*}\s*$/, '');
...
const realReturnIndex = lines.findIndex(l => l === '  return (');
if (realReturnIndex !== -1) {
    lines.splice(realReturnIndex, lines.length - realReturnIndex, newRender);
    fs.writeFileSync('apps/agent/src/pages/admin/FleetFinance.tsx', lines.join('\n'));
    console.log('Successfully patched!');
} else {
    console.log('Could not find exact return index.');
}
