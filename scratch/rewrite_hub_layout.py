...
layout_tsx_content = """import React, { useState } from 'react';
...
const NAV_CONFIG: NavSection[] = [
  ...
  },
  {
    ...
    icon: FileBarChart,
    items: [
      { name: 'Executive Intelligence', path: '/analytics/executive' },
      { name: 'Material Analytics', path: '/analytics/material' },
      { name: 'Collection Analytics', path: '/analytics/collection' },
      { name: 'Fleet Analytics', path: '/analytics/fleet' },
      { name: 'Procurement Analytics', path: '/analytics/procurement' },
      { name: 'Predictive Insights', path: '/analytics/predictive' },
    ]
  },
  {
...
export default function HubLayout() {
  ...
  return (
    <div className={`flex min-h-screen font-sans antialiased transition-colors duration-300 ${isDarkMode ? 'bg-surface-950 text-slate-300' : 'bg-surface-50 text-slate-600'}`}>
      
      {/* ── SIDEBAR ── */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 flex flex-col border-r transition-all duration-300
        ${isDarkMode ? 'bg-surface-900 border-white/5 shadow-[2px_0_10px_rgba(0,0,0,0.5)]' : 'bg-white border-slate-200 shadow-[2px_0_10px_rgba(0,0,0,0.05)]'}
        ${isSidebarOpen ? 'translate-x-0 w-72' : '-translate-x-full w-72 lg:translate-x-0'}
        ${isDesktopSidebarCollapsed ? 'lg:w-[72px]' : 'lg:w-72'}
      `}>
        {/* Sidebar Header */}
          ...
          {NAV_CONFIG.map((section) => (
            <div key={section.title} className={`w-full ${isDesktopSidebarCollapsed ? 'lg:flex lg:flex-col lg:items-center lg:mb-4' : 'mb-6'}`}>
              ...
              <button 
                ...
                className={`w-full flex items-center justify-between py-2 text-[10px] font-bold uppercase tracking-widest transition-colors
                  ${isDesktopSidebarCollapsed ? 'px-0 justify-center' : 'px-2'}
                  ${isDarkMode ? 'text-slate-500 hover:text-slate-300' : 'text-slate-400 hover:text-slate-600'}`}
              >
                <div className={`flex items-center gap-3 ${isDesktopSidebarCollapsed ? 'justify-center' : ''}`}>
                  <section.icon className={`w-4 h-4 shrink-0 ${isDesktopSidebarCollapsed ? 'opacity-70' : 'opacity-100'}`} />
                      ...
                      <NavLink
                        ...
                        className={`
                          ...
                          ${isActive 
                            ...
                            : (isDarkMode ? 'text-slate-400 hover:bg-surface-800/50 hover:text-slate-200' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900')
                          }
                        `}
                      >
        ...
        {/* User Profile */}
        <div className={`border-t shrink-0 ${isDarkMode ? 'border-white/5 bg-surface-900/50' : 'border-slate-100 bg-slate-50/50'} ${isDesktopSidebarCollapsed ? 'p-2 flex justify-center' : 'p-4'}`}>
      ...
      {/* ── MAIN CONTENT ── */}
      <main className={`flex-1 flex flex-col min-h-screen overflow-hidden transition-all duration-300 ${isDesktopSidebarCollapsed ? 'lg:ml-[72px]' : 'lg:ml-72'}`}>
        ...
        <header className={`h-16 shrink-0 flex items-center justify-between px-4 sm:px-6 border-b z-40 sticky top-0 backdrop-blur-md ${isDarkMode ? 'bg-surface-950/80 border-white/5 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.5)]' : 'bg-white/80 border-slate-200 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)]'}`}>
           ...
           <div className="flex items-center gap-4">
             <button 
               ...
               className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-surface-800' : 'hover:bg-slate-100'}`}
             >
               <Menu className="w-5 h-5" />
             </button>

             {/* Org Switcher */}
             <div className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg border cursor-pointer hover:border-emerald-500/50 transition-colors ${isDarkMode ? 'bg-surface-900 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
           ...
           <div className="flex-1 max-w-xl px-4 hidden lg:block">
             <div className={`relative flex items-center w-full rounded-full border transition-colors ${isDarkMode ? 'bg-surface-900/50 border-white/10 focus-within:border-emerald-500/50 focus-within:bg-surface-900' : 'bg-slate-50 border-slate-200 focus-within:border-emerald-500/50 focus-within:bg-white'}`}>
                ...
                <div className="absolute right-3 flex items-center gap-1">
                   <kbd className={`hidden xl:inline-block px-1.5 py-0.5 text-[10px] font-semibold rounded border ${isDarkMode ? 'bg-surface-800 border-white/10 text-slate-400' : 'bg-white border-slate-200 text-slate-400'}`}>⌘</kbd>
                   <kbd className={`hidden xl:inline-block px-1.5 py-0.5 text-[10px] font-semibold rounded border ${isDarkMode ? 'bg-surface-800 border-white/10 text-slate-400' : 'bg-white border-slate-200 text-slate-400'}`}>K</kbd>
                </div>
             </div>
           </div>
           ...
           <div className="flex items-center gap-2 sm:gap-3">
              <button className={`hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-semibold transition-colors ${isDarkMode ? 'bg-surface-900 border-white/10 hover:border-emerald-500/50 text-slate-300' : 'bg-white border-slate-200 hover:border-emerald-500/50 text-slate-600'}`}>
                 <FileBarChart className="w-3.5 h-3.5 text-emerald-500" />
                 Generate Report
              </button>
              <button className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-xs font-semibold hover:bg-indigo-500/20 transition-colors border border-indigo-500/20">
              ...
              <button 
                onClick={toggleTheme}
                className={`p-2 rounded-full transition-colors ${isDarkMode ? 'hover:bg-surface-800 text-slate-400 hover:text-yellow-400' : 'hover:bg-slate-100 text-slate-400 hover:text-indigo-600'}`}
              >
                {isDarkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
              </button>
              <button 
                className={`relative p-2 rounded-full transition-colors ${isDarkMode ? 'hover:bg-surface-800 text-slate-400' : 'hover:bg-slate-100 text-slate-400'}`}
              >
                 <Bell className="w-5 h-5" />
                 {unreadCount > 0 && (
                   <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-rose-500 rounded-full border-2 border-white dark:border-surface-950" />
                 )}
              </button>
           </div>
        </header>
