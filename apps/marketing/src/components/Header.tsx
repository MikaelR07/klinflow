            ...
            <AnimatePresence>
              {productsOpen && (
                <motion.div 
                  ...
                  onMouseEnter={() => setProductsOpen(true)}
                  onMouseLeave={() => setProductsOpen(false)}
                  className={`absolute top-full left-1/2 -translate-x-1/2 mt-4 w-[480px] p-4 rounded-md border shadow-xl ${isDarkMode ? 'bg-surface-900 border-white/5' : 'bg-white border-slate-100'}`}
                >
                  <div className="grid grid-cols-2 gap-2">
                    {productLinks.map((link) => (
                      <Link 
                        key={link.path} 
                        to={link.path}
                        className={`p-4 rounded-md transition-all ${isDarkMode ? 'hover:bg-white/5' : 'hover:bg-slate-50'}`}
                      >
              ...
              <div className="grid gap-6">
                ...
                {productLinks.map((link) => (
                  <Link key={link.path} to={link.path} className="flex items-center gap-3 sm:gap-4">
                    <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-md flex items-center justify-center shrink-0 ${isDarkMode ? 'bg-white/5' : 'bg-slate-100'}`}>
