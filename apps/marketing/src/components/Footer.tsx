      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-20">
          <div className="lg:col-span-2">
            ...
            <p className={`font-medium leading-relaxed max-w-sm italic mb-8 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
              The financial engine for the circular economy. Transforming Africa's waste management into a high-yield digital asset ecosystem.
            </p>
            <div className="flex items-center gap-6">
              <Globe className={`w-5 h-5 transition-all cursor-pointer ${isDarkMode ? 'text-slate-300 hover:text-emerald-500' : 'text-slate-500 hover:text-emerald-500'}`} />
              <Mail className={`w-5 h-5 transition-all cursor-pointer ${isDarkMode ? 'text-slate-300 hover:text-emerald-500' : 'text-slate-500 hover:text-emerald-500'}`} />
              <MessageSquare className={`w-5 h-5 transition-all cursor-pointer ${isDarkMode ? 'text-slate-300 hover:text-emerald-500' : 'text-slate-500 hover:text-emerald-500'}`} />
            </div>
          </div>
              ...
              <ul className="space-y-4">
                {section.links.map((link) => (
                  <li key={link.name}>
                    <Link 
                      to={link.path} 
                      className={`text-sm font-bold transition-colors ${isDarkMode ? 'text-slate-400 hover:text-emerald-500' : 'text-slate-500 hover:text-emerald-500'}`}
                    >
                      {link.name}
                    </Link>
                  </li>
        ...
        <div className="pt-12 border-t border-slate-200 dark:border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500 italic">© 2026 Klinflow Logistics. All Rights Reserved.</p>
          <div className="flex gap-8 text-xs font-bold uppercase tracking-widest text-slate-500">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="hover:text-emerald-500 transition-colors cursor-pointer">System: Operational</span>
            </div>
          </div>
        </div>
      </div>
