...
export default function DownloadSection({ isDarkMode }: { isDarkMode: boolean }) {
  return (
    <section className={`py-40 px-6 relative overflow-hidden ${isDarkMode ? 'bg-surface-950' : 'bg-slate-50'}`}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          <h2 className={`text-4xl md:text-7xl font-bold tracking-tighter mb-8 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
            Klinflow in Your <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary-dark">Hand & Workspace.</span>
          </h2>
          ...
          {appSuite.map((app, i) => (
            <motion.div
              ...
              className={`p-10 rounded-[3rem] border flex flex-col h-full transition-all duration-500 ${
                isDarkMode 
                  ? 'bg-surface-900 border-white/5 hover:border-primary/20 shadow-2xl' 
            ...
            >
              ...
              <div className="mb-4">
                <span className="text-xs font-black text-primary uppercase tracking-widest mb-1 block">{app.type}</span>
                ...
                {app.badges ? (
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                      <button className="flex items-center gap-3 px-6 py-3 bg-surface-950 dark:bg-white text-white dark:text-slate-900 rounded-xl hover:scale-105 transition-all shadow-lg group">
