      ...
      <section className="relative pt-24 pb-20 md:pt-32 md:pb-32 overflow-hidden min-h-[70vh] md:min-h-[85vh] flex items-center">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          ...
          <motion.h1
            ...
            className={`text-4xl md:text-5xl lg:text-6xl font-bold tracking-tighter mb-8 max-w-5xl mx-auto leading-[1.1] ${isDarkMode ? "text-white" : "text-slate-900"}`}
          >
          ...
          >
            AI-powered coordination for waste collectors, recyclers, and
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            ...
            >
              ...
              >
                Klinflow is more than an app—it is the digital foundation for a
                circular society. By integrating real-time telemetry, automated
                rewards, and industrial marketplaces, we are transforming waste
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {[
                  ...
                  <div
                    key={i}
                    className={`p-6 rounded-2xl border ${isDarkMode ? "bg-white/5 border-white/5" : "bg-slate-50 border-slate-200"}`}
                  >
                    <div className="text-2xl font-black text-primary mb-1">
                      {stat.val}
                    </div>
                    <div
                      className={`text-xs font-bold uppercase tracking-widest mb-1 ${isDarkMode ? "text-white" : "text-slate-900"}`}
                    >
                      {stat.label}
                    </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row items-end justify-between gap-8 mb-20">
            <div className="max-w-2xl text-left">
              <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-primary mb-6">
                The System Lifecycle
              </h2>
              <h3
                className={`text-3xl md:text-4xl font-bold tracking-tighter ${isDarkMode ? "text-white" : "text-slate-900"}`}
              >
                A Trust-Less, <br className="hidden sm:block" />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary-dark italic">
                  Self-Liquidating Ledger.
                </span>
              </h3>
            </div>
            <p
              className={`text-base md:text-lg font-medium max-w-sm ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}
            >
              Every transaction in Klinflow is verified by AI and settled
              instantly, creating a friction-free circular economy.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {loopSteps.map((step) => (
              <div
                key={step.id}
                className={`p-8 rounded-2xl border transition-all ${isDarkMode ? "bg-surface-900 border-white/5 hover:border-primary/20" : "bg-slate-50 border-slate-100 hover:border-primary/20 shadow-sm"}`}
              >
                <div
                  className={`w-12 h-12 rounded-2xl ${step.bg} ${step.color} flex items-center justify-center mb-6`}
                >
                  <step.icon className="w-6 h-6" />
                </div>
                <h4
                  className={`text-xl font-bold mb-3 ${isDarkMode ? "text-white" : "text-slate-900"}`}
                >
                  {step.title}
                </h4>
      ...
      >
        <div className="max-w-7xl mx-auto px-6">
            ...
            {screenshots.map((shot, idx) => (
              ...
              >
                <div
                  ...
                  className={`group relative rounded-2xl border overflow-hidden cursor-pointer transition-all duration-500 ${isDarkMode ? "border-white/5 bg-surface-950" : "border-slate-200 bg-white shadow-xl hover:shadow-2xl"}`}
                >
                  <img
                    src={shot.src}
                    alt={shot.alt}
          ...
          <div className="mt-16 flex justify-center">
            <Link
              to="/gallery"
              className={`group px-8 py-4 rounded-2xl font-bold text-sm uppercase tracking-widest flex items-center gap-3 transition-all active:scale-95 ${isDarkMode
                ? "bg-white/5 border border-white/10 text-white hover:bg-white/10"
                : "bg-white border border-slate-200 text-slate-900 hover:bg-slate-50 shadow-lg hover:shadow-xl"
                }`}
            >
              View Suite Gallery
              <div className="w-8 h-8 rounded-xl bg-primary text-white flex items-center justify-center group-hover:translate-x-1 transition-transform">
          ...
          <div>
            ...
            <div className="grid gap-4">
              {[
                {
                  title:
                    "Residents schedule pickups, receive weight-based valuation, and track payouts in real time via GreenFlow Points",
                  icon: Shield,
                },
                {
                  title:
                    "Agents leverage AI grading and multi-stop route optimization to maximize hourly commission yield.",
                  icon: LineChart,
                },
                {
                  title:
                    "Industrial buyers secure large material lots through transparent B2B escrow and logistics tracking.",
                  icon: Handshake,
                },
              ].map((f, i) => (
                <div
