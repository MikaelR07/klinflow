    ...
    <Layout>
      <section className="relative pt-24 md:pt-32 pb-16 md:pb-32 overflow-hidden">
        <div className="absolute inset-0 z-0">
          ...
          <div className={`absolute top-0 right-0 w-[500px] h-[500px] bg-primary/10 blur-[120px] opacity-30 rounded-full`} />
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 text-left">
             {[
                ...
                <div key={i} className={`p-10 rounded-2xl border ${isDarkMode ? 'bg-surface-950 border-white/5' : 'bg-white border-slate-100 shadow-xl'}`}>
      ...
      <section className="py-16 md:py-32 px-6">
         <div className={`max-w-5xl mx-auto p-8 md:p-20 rounded-2xl text-center ${isDarkMode ? 'bg-surface-950' : 'bg-slate-50'}`}>
            <h2 className={`text-3xl md:text-4xl font-bold tracking-tighter mb-8 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Ready to Clean Up?</h2>
            <button className="px-10 py-5 bg-primary text-white font-bold rounded-2xl shadow-xl shadow-emerald-500/20 hover:bg-primary transition-all">
