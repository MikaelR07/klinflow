  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-slate-100 dark:bg-slate-900">
      ...
      <motion.div
        ...
        className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 shadow-[0_-20px_50px_rgba(0,0,0,0.15)] rounded-t-[1rem] p-2 pb-8 cursor-grab active:cursor-grabbing"
      >
        <button
          onClick={() => setIsExpanded(!isExpanded)}
          className="w-full flex flex-col items-center pb-6 group"
        >
          <div className="w-16 h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full mb-2 group-hover:bg-emerald-500 transition-colors" />
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] group-hover:text-emerald-500 transition-colors">
            {isExpanded ? 'Slide Down to Map' : 'Slide Up for Details'}
          </div>
        </button>

        <div className="max-w-md mx-auto">
          {/* Header row with seller and phone */}
          <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-800/50 p-3 pr-4 rounded-2xl border border-slate-100 dark:border-slate-800 mb-4 shadow-sm">
            <div className="flex items-center gap-3.5">
              <div className="w-12 h-12 bg-white dark:bg-slate-900 rounded-xl flex items-center justify-center text-emerald-600 dark:text-emerald-400 font-black text-xl shadow-sm border border-slate-100 dark:border-slate-700/50">
                {sellerName.charAt(0)}
              </div>
              <div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Pickup From</p>
                <h3 className="text-sm font-black text-slate-900 dark:text-white leading-none tracking-tight">{sellerName}</h3>
              </div>
            </div>
            <button
              onClick={() => window.location.href = `tel:${sellerPhone}`}
              className="w-11 h-11 rounded-xl bg-emerald-500 text-white flex items-center justify-center active:scale-95 transition-all shadow-md shadow-emerald-500/20 shrink-0"
            >
          ...
          {/* Details grid */}
          <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 mb-4 shadow-sm">
            <div className="grid grid-cols-2 gap-y-5 gap-x-4">
              <div className="flex items-start gap-3">
                <MapPin className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />
                <div>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Location</p>
                  <p className="text-xs font-black text-slate-900 dark:text-white capitalize leading-tight">{location}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-black text-slate-900 dark:text-white capitalize leading-tight">{material}</p>
                </div>
              </div>
              <div className="flex items-start gap-3 col-span-2 pt-1">
                <Clock className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />
                <div>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Scheduled Date</p>
                  <p className="text-xs font-black text-slate-900 dark:text-white capitalize leading-tight">{order.scheduled_date ? new Date(order.scheduled_date).toLocaleDateString() : 'ASAP'}</p>
                </div>
              </div>
            </div>
          </div>

          <div className="bg-emerald-600 dark:bg-emerald-900/30 p-5 rounded-2xl border border-emerald-500/30 dark:border-emerald-800/50 shadow-md shadow-emerald-500/10 mb-6 flex items-start gap-3">
            <Truck className="w-5 h-5 text-white/90 dark:text-emerald-400 shrink-0 mt-0.5" />
            <div className="flex flex-col">
              <span className="text-[9px] font-bold uppercase tracking-widest text-emerald-100 dark:text-emerald-500 mb-1">Seller Notes</span>
              <span className={`text-[13px] font-medium leading-relaxed italic ${order.proposal?.notes ? 'text-white' : 'text-emerald-100/70 dark:text-emerald-400/60'}`}>
                {order.proposal?.notes ? `"${order.proposal.notes}"` : 'No notes provided.'}
              </span>
            </div>
          </div>
            ...
            {!hasArrived ? (
              <div className="space-y-4">
                <button
                  ...
                  className="w-full py-5 bg-emerald-500 text-white font-semibold text-sm rounded-xl flex items-center justify-center gap-3 active:scale-95 transition-all group tracking-widest"
                >
              ...
              <div className="flex flex-col gap-3 pt-2 pb-4">
                <button
                  ...
                  className="w-full py-5 bg-amber-500 text-white font-semibold text-sm rounded-xl flex items-center justify-center gap-3 active:scale-95 transition-all group"
                >
