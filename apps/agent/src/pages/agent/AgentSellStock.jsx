...
import { useAuthStore, useAgentStore, WASTE_CATEGORIES, supabase, compressImage } from '@cleanflow/core';
...
export default function AgentSellStock() {
  ...
  const availableCategories = useMemo(() => {
    ...
    const custom = profile?.service_profile?.custom_services || [];
  ...
  const handleSubcategoryChange = (subLabel) => {
    ...
    setFormData(prev => ({ 
      ...
      minOrder: (profile?.service_profile?.min_weight || 10).toString()
    }));
  };
