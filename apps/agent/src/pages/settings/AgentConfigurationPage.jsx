...
import { useAgentStore, useAuthStore, useServiceStore, WASTE_CATEGORIES, getCategoryBySlug, supabase } from '@cleanflow/core';
...
export default function AgentConfigurationPage() {
  ...
  const [hubData, setHubData] = useState({
    active: profile?.is_hub_active || false,
    address: profile?.hub_address || '',
    coords: profile?.hub_location || null
  });

  const isIndividualAgent = profile?.role === 'agent' && profile?.agent_account_type === 'individual';
  const isCompanyOwner = profile?.role === 'agent' && profile?.agent_account_type === 'company_owner';
  ...
  return (
    <div className="h-[calc(100dvh-40px)] flex flex-col bg-[#F8F8FF] dark:bg-slate-900 -mx-4 -mt-5">
      {/* Header */}
      <div className="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-white/5 p-4 flex items-center gap-4">
        <button 
          onClick={() => navigate(-1)} 
          className="p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl active:scale-95 transition-all text-slate-600 dark:text-slate-300"
        >
