...
import { supabase, useAuthStore, getCategoryBySlug, getSubcategoryLabel, usePriceStore } from '@cleanflow/core';
...
export default function Inventory() {
  ...
  // Navigation State
  const [viewLevel, setViewLevel] = useState('category'); // 'category', 'subcategory', 'asset'
  ...
  const categoryData = useMemo(() => {
    const groups = allAssets.reduce((acc, asset) => {
      const type = asset.material_type || 'Other';
      if (!acc[type]) {
      ...
      acc[type].value += (parseFloat(asset.weight_kg) || 0) * rate;
      acc[type].count += 1;
  ...
  const subcategoryData = useMemo(() => {
    ...
    const groups = selectedCategory.assets.reduce((acc, asset) => {
      ...
      acc[sub].weight += parseFloat(asset.weight_kg) || 0;
      acc[sub].value += (parseFloat(asset.weight_kg) || 0) * rate;
      acc[sub].count += 1;
