...
export default function HubDashboard({ onNavigate }) {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      let driverIds = [profile.id];
      if (profile.agent_account_type === 'company_admin') {
        const { data: drivers } = await supabase
          .from('profiles')
          .select('id')
          .eq('company_id', profile.id);
        driverIds = [profile.id, ...(drivers?.map(d => d.id) || [])];
      }
      
      // Ensure driverIds only contains valid UUID strings to prevent 400 Bad Request
      driverIds = [...new Set(driverIds)].filter(id => id && typeof id === 'string' && id.length > 20);
      ...
      // 2. Fetch Actual Inventory (Sum of all assets physically inside this specific hub)
      ...
      const totalInventory = hubAssets?.reduce((acc, curr) => acc + (curr.weight_kg || 0), 0) || 0;
      setInventoryWeight(totalInventory);

      // 3. Fetch Recent Logistics (Grouped by Agent Visit)
      const { data: recent, error: recentError } = await supabase
        ...
        .limit(20); // Get more to allow grouping

      if (recentError) throw recentError;

      if (recent && recent.length > 0) {
        // GROUPING LOGIC: Group assets by the exact same second of transfer (or by session)
        const groups = recent.reduce((acc, asset) => {
          const agentId = asset.verifier_id;
          const timeKey = new Date(asset.created_at).setSeconds(0,0); // Group by minute
          ...
          if (!acc[key]) {
            acc[key] = {
              ...
              totalWeight: 0,
              time: new Date(asset.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
              date: new Date(asset.created_at).toLocaleDateString(),
              materials: []
            };
          }
          acc[key].totalWeight += asset.weight_kg;
          acc[key].materials.push({
            id: asset.id,
            type: asset.material_type,
            weight: asset.weight_kg,
            grade: asset.grade
          });
          return acc;
        }, {});
      ...
      // 4. Fetch Active Personnel & Real-Time Radar Alerts (En Route)
      ...
      setActivePersonnel(activeDrivers?.length || 0);

      if (activeDrivers && activeDrivers.length > 0) {
        // Create live radar alerts for drivers who are on their way
        const alerts = await Promise.all(activeDrivers.map(async (d) => {
          // Attempt to sum their verified load to show what's coming
          const { data: driverLoad } = await supabase
             .from('assets')
             .select('weight_kg')
             .eq('verifier_id', d.id)
             .eq('status', 'verified');
          const incomingWeight = driverLoad?.reduce((acc, curr) => acc + (curr.weight_kg || 0), 0) || 0;

          return {
            id: d.id,
            agent: d.name,
            profile_picture_url: d.avatar_url,
      ...
      console.error('[Hub Dashboard] Error fetching data:', error);
  ...
  useEffect(() => {
    ...
    // ── REAL-TIME RADAR: Subscribing to incoming agent signals ──
    const radarChannel = supabase.channel('hub-radar-sync')
      .on('postgres_changes', { 
        event: 'UPDATE', 
        schema: 'public', 
        table: 'profiles',
        // If company_admin, listen for any driver in their company. Otherwise just themselves.
        filter: profile?.agent_account_type === 'company_admin' ? `company_id=eq.${profile.id}` : `id=eq.${profile.id}`
      }, (payload) => {
        console.log('[Radar] Incoming change detected:', payload.new.name, 'En Route:', payload.new.is_en_route);
        fetchDashboardData();
      })
      .on('postgres_changes', {
        event: '*',
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') console.log('[Radar] Hub Radar is Live & Listening...');
      });
  ...
  const AgentVisitCard = ({ visit, defaultOpen = false }) => {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight text-slate-900 dark:text-white">
            {profile?.agent_account_type === 'company_admin' ? `${profile?.name}'s Fleet Hub` : 'Global Verification Hub'}
          </h1>
