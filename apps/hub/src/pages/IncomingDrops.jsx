  ...
  const fetchIncoming = async () => {
    try {
      ...
      const enriched = await Promise.all((agents || []).map(async (agent) => {
        ...
        const totalWeight = (assets || []).reduce((acc, a) => acc + (a.weight_kg || 0), 0);
        const materials = Array.from(new Set((assets || []).map(a => a.material_type))).join(', ');
        ...
        return {
          ...
          progress: 80 // We could use location distance here later
        };
      }));
