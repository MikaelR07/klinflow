...
const MOCK_SPEND = [
  { name: 'Logistics', value: 2100000, max: 6000000, color: 'bg-fuchsia-500' },
  { name: 'Agent Payouts', value: 5800000, max: 6000000, color: 'bg-fuchsia-500' },
  { name: 'Operations', value: 1200000, max: 6000000, color: 'bg-fuchsia-500' },
  { name: 'Equipment', value: 800000, max: 6000000, color: 'bg-fuchsia-500' },
];
    ...
    <div className="flex h-full w-full relative bg-transparent">
        ...
        <div className="max-w-[1600px] mx-auto space-y-5">
          ...
          <div>
             <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2">
               {[
                 { label: 'Total Materials Collected', val: '1,425', unit: 'Tons', icon: Factory, color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-500/10', trend: '+15.2%', trendUp: true },
                 { label: 'Collected Material Value', val: 'KES 42.5M', unit: '', icon: Wallet, color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-500/10', trend: '+8.4%', trendUp: true },
                 { label: 'Total Hub Dropoffs', val: '8,420', unit: '', icon: Warehouse, color: 'text-fuchsia-500', bg: 'bg-fuchsia-50 dark:bg-fuchsia-500/10', trend: '+12.1%', trendUp: true },
                 { label: 'Hub Collection', val: '845', unit: 'Tons', icon: Scale, color: 'text-violet-500', bg: 'bg-violet-50 dark:bg-violet-500/10', trend: '-2.4%', trendUp: false },
                 { label: 'RFQs Won', val: '142', unit: '', icon: FileText, color: 'text-pink-500', bg: 'bg-pink-50 dark:bg-pink-500/10', trend: '+5.0%', trendUp: true },
                 { label: 'Transactions', val: '12,450', unit: '', icon: Activity, color: 'text-teal-500', bg: 'bg-teal-50 dark:bg-teal-500/10', trend: '+22.4%', trendUp: true },
               ].map((kpi, i) => (
                 <Card key={i} className="!p-4">
                   ...
                   <h2 className="text-lg font-bold text-bg-slate-600 dark:text-white tracking-tighter">
                     {kpi.val} {kpi.unit && <span className="text-xs ml-1 font-semibold text-slate-500 dark:text-slate-400">{kpi.unit}</span>}
                   </h2>
                   <div className="mt-2 flex items-center gap-1.5">
                     ...
                     <span className="font-bold text-[10px] text-slate-600 dark:text-slate-500">vs yesterday</span>
                   </div>
                 </Card>
               ))}
             </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2">
            {[
              ...
              { label: 'Completed Pickups', val: '24,150', unit: '', icon: ShieldCheck, color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-500/10', trend: '+8.1%', trendUp: true },
              { label: 'Agent Complaints', val: '14', unit: '', icon: AlertCircle, color: 'text-rose-500', bg: 'bg-rose-50 dark:bg-rose-500/10', trend: '-12.0%', trendUp: true },
              ...
              { label: 'Fleet Rating', val: '4.8', unit: '/5.0', icon: Zap, color: 'text-teal-500', bg: 'bg-teal-50 dark:bg-teal-500/10', trend: '+0.1', trendUp: true },
                 ...
                 <Card key={i} className="!p-4">
                   ...
                   <h2 className="text-lg font-bold text-[#131722] dark:text-white tracking-tighter">
                     {kpi.val} {kpi.unit && <span className="text-xs ml-1 font-semibold text-slate-500 dark:text-slate-400">{kpi.unit}</span>}
                   </h2>
                   <div className="mt-2 flex items-center gap-1.5">
                     <TrendingUp className={`w-3 h-3 ${kpi.trendUp ? 'text-emerald-500' : 'text-rose-500'} ${kpi.trendUp ? '' : 'rotate-180'}`} />
          ...
          {/* ROW 3: Analytics Visualizations (12 Column Grid for precise control) */}
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-3 lg:gap-2">
            {/* Multi-line chart (Col span 4) */}
            ...
            {/* Collection by Location Native HTML Bars (Col span 2) */}
            <Card className="xl:col-span-3 flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto mt-2 space-y-3 pr-2 scrollbar-hide">
                 {MOCK_LOCATION_RANKING.map((loc, i) => (
                   <div key={i} className="w-full">
                     <div className="flex justify-between items-end mb-1">
                       <span className={`text-[10px] font-bold uppercase tracking-widest ${textSub}`}>{loc.name}</span>
                       <span className={`text-[10px] font-bold ${textTitle}`}>{loc.value.toLocaleString()} Kg</span>
                     </div>
                     <div className={`w-full h-4 rounded-full overflow-hidden ${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
                       <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${(loc.value / loc.max) * 100}%` }}></div>
                     </div>
                   </div>
                 ))}
              </div>
            ...
            {/* Weekly Payouts (Col span 3) */}
            <Card className="xl:col-span-3 flex flex-col">
              <SectionHeader icon={Wallet} title="Weekly Payouts" />
              <div className="flex-1 h-[250px] w-full mt-2">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={MOCK_PAYOUTS} margin={{ top: 0, right: 0, left: -28, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={isDarkMode ? '#1e293b' : '#f1f5f9'} vertical={false} />
                  ...
                  <tbody className="text-xs">
                    {MOCK_TOP_AGENTS.map((agent, i) => (
                      <tr key={i} className={`border-b last:border-0 ${isDarkMode ? 'border-slate-700/30' : 'border-slate-100'}`}>
                        <td className={`py-3 font-bold truncate max-w-[120px] ${textTitle}`}>{agent.name}</td>
                        <td className={`py-3 text-right font-bold ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>{agent.kg}</td>
                        <td className={`py-3 text-right font-bold ${textSub}`}>{agent.pickups}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-3 lg:gap-4">
            ...
            <Card className="flex flex-col items-center justify-between relative">
              ...
              <div className="h-[180px] w-full relative">
                <ResponsiveContainer width="100%" height="100%">
                  <RePieChart>
                    <Pie data={MOCK_FLEET_STATUS} cx="50%" cy="50%" innerRadius={55} outerRadius={75} paddingAngle={2} dataKey="value" stroke="none">
            ...
            <Card className="flex flex-col">
              ...
              <div className="flex-1 h-[200px] w-full mt-2">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={MOCK_PICKUPS_DAILY} margin={{ top: 0, right: 0, left: -40, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={isDarkMode ? '#1e293b' : '#f1f5f9'} vertical={false} />
            ...
            <Card className="flex flex-col items-center justify-between relative">
              ...
              <div className="h-[180px] w-full relative">
                <ResponsiveContainer width="100%" height="100%">
                  <RePieChart>
                    <Pie data={MOCK_MATERIAL_COMPOSITION} cx="50%" cy="50%" innerRadius={0} outerRadius={75} paddingAngle={0} dataKey="value" stroke="none">
            ...
            <Card className="flex flex-col">
              ...
              <div className="flex-1 h-[200px] w-full mt-2">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={MOCK_REVENUE_GROWTH} margin={{ top: 0, right: 0, left: -22, bottom: 0 }}>
                    <defs>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
              {[
                { label: 'CO₂ Saved', val: '4,520', unit: 'Tons', icon: Globe, color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-500/10', trend: '+12.4%', trendUp: true },
                { label: 'Landfill Diversion', val: '92.4', unit: '%', icon: RecycleIcon, color: 'text-indigo-500', bg: 'bg-indigo-50 dark:bg-indigo-500/10', trend: '+1.2%', trendUp: true },
                { label: 'Trees Equivalent', val: '12,400', unit: '', icon: Leaf, color: 'text-teal-500', bg: 'bg-teal-50 dark:bg-teal-500/10', trend: '+450', trendUp: true },
                { label: 'Communities Served', val: '142', unit: '', icon: Users, color: 'text-amber-500', bg: 'bg-amber-50 dark:bg-amber-500/10', trend: '+5', trendUp: true },
                { label: 'Water Saved', val: '8.2M', unit: 'Liters', icon: DropletIcon, color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-500/10', trend: '+500K', trendUp: true },
                { label: 'Energy Recovered', val: '450', unit: 'MWh', icon: Zap, color: 'text-rose-500', bg: 'bg-rose-50 dark:bg-rose-500/10', trend: '+12%', trendUp: true },
              ].map((esg, i) => (
                <Card key={i} className="!p-4">
                   <div className="flex items-center gap-2 mb-3">
                     <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${esg.bg}`}>
                       <esg.icon className={`w-4 h-4 ${esg.color}`} />
                     </div>
                     <p className="font-bold text-[10px] text-slate-600 dark:text-slate-500 uppercase tracking-widest truncate">{esg.label}</p>
                   </div>
                   <h2 className="text-lg font-bold text-[#131722] dark:text-white tracking-tighter">
                     {esg.val} {esg.unit && <span className="text-xs ml-1 font-semibold text-slate-500 dark:text-slate-400">{esg.unit}</span>}
                   </h2>
                   <div className="mt-2 flex items-center gap-1.5">
                     <TrendingUp className={`w-3 h-3 ${esg.trendUp ? 'text-emerald-500' : 'text-rose-500'} ${esg.trendUp ? '' : 'rotate-180'}`} />
                     <span className={`text-[10px] ${esg.trendUp ? 'text-emerald-500' : 'text-rose-500'}`}>{esg.trend}</span>
                     <span className="font-bold text-[10px] text-slate-600 dark:text-slate-500">vs yesterday</span>
                   </div>
                 </Card>
              ))}
            ...
            <Card className="flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto mt-2 space-y-3 pr-2 scrollbar-hide">
                 {MOCK_RFQ_PIPELINE.map((rfq, i) => (
                   <div key={i} className="w-full">
                     <div className="flex justify-between items-end mb-1">
                       <span className={`text-[10px] font-bold uppercase tracking-widest ${textSub}`}>{rfq.name}</span>
                       <span className={`text-[10px] font-bold ${textTitle}`}>{rfq.value.toLocaleString()}</span>
                     </div>
                     <div className={`w-full h-2.5 rounded-full overflow-hidden ${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
            ...
            <Card className="flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto mt-2 space-y-3 pr-2 scrollbar-hide">
                 {MOCK_TOP_BUYERS.map((buyer, i) => (
                   <div key={i} className="w-full">
                     <div className="flex justify-between items-end mb-1">
                       <span className={`text-[10px] font-bold uppercase tracking-widest ${textSub}`}>{buyer.name}</span>
                       <span className={`text-[10px] font-bold ${textTitle}`}>KES {(buyer.value/1000000).toFixed(1)}M</span>
                     </div>
                     <div className={`w-full h-2.5 rounded-full overflow-hidden ${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
          ...
          <div className="grid xl:grid-cols-4 gap-3 lg:gap-4 pb-12">
            ...
            <Card className="flex flex-col items-center justify-between relative">
              ...
              <div className="h-[180px] w-full mt-4 relative">
                <ResponsiveContainer width="100%" height="100%">
                  <RePieChart>
                    <Pie data={MOCK_PROFITABILITY} cx="50%" cy="50%" innerRadius={55} outerRadius={75} paddingAngle={2} dataKey="profit" stroke="none">
            ...
            {/* Profitability Analysis (Horizontal Bars via layout=vertical) */}
            <Card className="flex flex-col">
              <SectionHeader icon={Activity} title="Profitability Margin (%)" />
              <div className="flex-1 h-[200px] w-full mt-2">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={MOCK_PROFITABILITY} layout="vertical" margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={isDarkMode ? '#1e293b' : '#f1f5f9'} horizontal={false} />
                    <XAxis type="number" hide />
                    <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#94a3b8' }} width={40} />
                    <Tooltip cursor={{ fill: 'transparent' }} content={<CustomTooltip />} />
                    <Bar dataKey="margin" fill="#10b981" radius={[0, 4, 4, 0]} barSize={15} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </Card>

            {/* Procurement Spend HTML Bars */}
            <Card className="flex flex-col">
              <SectionHeader icon={Wallet} title="Procurement Spend" />
              <div className="flex-1 overflow-y-auto mt-2 space-y-3 pr-2 scrollbar-hide">
                 {MOCK_SPEND.map((spend, i) => (
                   <div key={i} className="w-full">
                     <div className="flex justify-between items-end mb-1">
                       <span className={`text-[10px] font-bold uppercase tracking-widest ${textSub}`}>{spend.name}</span>
                       <span className={`text-[10px] font-bold ${textTitle}`}>KES {(spend.value/1000000).toFixed(1)}M</span>
                     </div>
                     <div className={`w-full h-2.5 rounded-full overflow-hidden ${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
                       <div className={`h-full ${spend.color} rounded-full`} style={{ width: `${(spend.value / spend.max) * 100}%` }}></div>
                     </div>
                   </div>
                 ))}
              </div>
            ...
            <Card className="flex flex-col">
              ...
              <div className="flex-1 h-[200px] w-full mt-2">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={MOCK_AGENT_VS_HUB} margin={{ top: 5, right: 0, left: -24, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={isDarkMode ? '#1e293b' : '#f1f5f9'} vertical={false} />
                    <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#94a3b8' }} dy={10} />
                    <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#94a3b8' }} />
                    <Tooltip content={<CustomTooltip />} />
      ...
      <div className={`absolute top-0 right-0 h-full transition-transform duration-300 z-30 flex ${isSidebarOpen ? 'translate-x-0' : 'translate-x-[320px]'}`}>
        ...
        <button 
          ...
          className={`absolute top-8 -left-8 w-8 h-16 flex items-center justify-center rounded-l-xl border-y border-l shadow-[-4px_0_15px_-5px_rgba(0,0,0,0.1)] transition-colors
