...
export default function HubSettings() {
  ...
  const [formData, setFormData] = useState({
    hubName: profile?.company_name || '',
    address: profile?.hub_config?.address || '',
    contactEmail: profile?.hub_config?.contactEmail || profile?.email || '',
    contactPhone: profile?.hub_config?.contactPhone || profile?.phone || '',
    capacityKg: profile?.hub_config?.capacityKg || '5000',
    operatingHours: profile?.hub_config?.operatingHours || 'Mon - Sat: 08:00 - 18:00',
    minWeight: profile?.service_profile?.min_weight || 5,
    maxWeight: profile?.service_profile?.max_weight || 500,
    supportedCategories: profile?.service_profile?.categories || []
  });

  // Fetch standard categories to allow selection
  useEffect(() => {
  ...
  const handleToggleCategory = (catLabel) => {
    setFormData(prev => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const payload = {
        ...
        service_profile: {
          min_weight: parseFloat(formData.minWeight),
          max_weight: parseFloat(formData.maxWeight),
    ...
    <div className="space-y-8 animate-fade-in pb-10 max-w-6xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          ...
          className="px-5 py-2.5 bg-rose-50 dark:bg-rose-500/10 text-rose-600 rounded-2xl text-[10px] font-semibold uppercase tracking-widest hover:bg-rose-100 transition-all flex items-center gap-2 w-fit shadow-sm"
        >
        ...
        <div className="md:col-span-1 space-y-2">
          {tabs.map(tab => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-5 py-4 rounded-3xl text-[10px] font-semibold uppercase tracking-[0.1em] transition-all ${
                activeTab === tab.id 
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-200 dark:border-white/5 p-8 shadow-sm">
            ...
            {activeTab === 'general' && (
              <form onSubmit={handleSave} className="space-y-6 animate-fade-in">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                     <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-2">Facility Branding Name (Company)</label>
                     <input
                       type="text"
                       name="hubName"
                  ...
                  <div>
                     <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-2">Total Storage Capacity (KG)</label>
                     <input
                       type="number"
                       name="capacityKg"
                ...
                <div className="pt-8 mt-8 border-t border-slate-100 dark:border-white/5 flex justify-end">
                  <button 
                    type="submit"
                    disabled={isSaving}
                    className="px-10 py-5 bg-primary text-white rounded-[2rem] font-semibold text-[10px] uppercase tracking-widest shadow-xl shadow-primary/20 transition-all flex items-center gap-3 disabled:opacity-50"
                  >
            ...
            {activeTab === 'service' && (
              <form onSubmit={handleSave} className="space-y-8 animate-fade-in">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div>
                      <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-2">Minimum Collection Weight (KG)</label>
                      <input
                        type="number"
                        name="minWeight"
                      ...
                      />
                      <p className="text-[9px] text-slate-400 mt-2">Pickups below this weight will be ignored by matching.</p>
                   </div>
                   <div>
                      <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-2">Maximum Fleet Capacity (KG)</label>
                      <input
                        type="number"
                        name="maxWeight"
                        ...
                        className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/5 rounded-2xl px-5 py-4 text-sm font-semibold focus:ring-2 focus:ring-indigo-500 outline-none"
                      />
                      <p className="text-[9px] text-slate-400 mt-2">Maximum weight your fleet can handle in a single trip.</p>
                   </div>
                </div>

                <div className="space-y-4">
                  <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Supported Material Categories</label>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p className={`text-[9px] font-semibold ${isActive ? 'text-white/70' : 'text-slate-400'}`}>Enable for fleet collection</p>
                          </div>
                ...
                <div className="pt-8 mt-8 border-t border-slate-100 dark:border-white/5 flex justify-end">
                   <button 
                     type="submit" 
                     disabled={isSaving}
                     className="px-10 py-5 bg-indigo-600 text-white rounded-[2rem] font-semibold text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-500/20 hover:scale-[1.02] transition-all flex items-center gap-3 disabled:opacity-50"
                   >
            ...
            {activeTab === 'location' && (
              <form onSubmit={handleSave} className="space-y-8 animate-fade-in">
                <div className="grid grid-cols-1 gap-8">
                  <div>
                    <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-3">Facility Physical Address</label>
                    <textarea
                      name="address"
                  ...
                  <div>
                    <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-2">Public Operating Hours</label>
                    <input
                      type="text"
                      name="operatingHours"
