  ...
  const handleAdminRegister = async (e) => {
    ...
    try {
      await register({
        ...
        pin: formData.pin,
        role: ROLES.ADMIN,
        location: { estate: 'System', latitude: null, longitude: null } 
      });
        ...
        <form onSubmit={handleAdminRegister} className="bg-slate-800 p-6 sm:p-8 rounded-3xl border border-slate-700 shadow-2xl space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="admin@cleanflow.app" className="w-full pl-11 pr-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white focus:ring-2 text-base focus:ring-rose-500/50 text-sm" />
            </div>
          </div>
