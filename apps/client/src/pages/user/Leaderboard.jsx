  ...
  const MerchantPodiumSlot = ({ user, height, bgClass, isFirst }) => (
    <div className="flex flex-col items-center flex-1">
      ...
      <div className={`w-full ${user ? bgClass : 'bg-slate-100 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700'} rounded-2xl ${height} flex flex-col items-center justify-center relative shadow-2xl`}>
        {user ? (
          <>
            <p className={`text-xs font-black ${isFirst ? 'text-amber-400' : 'text-slate-400'} uppercase tracking-widest truncate w-20 text-center mt-2`}>{user.name}</p>
          ...
          <p className="text-xs font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest">Open</p>
        )}
      </div>
    </div>
  );
    ...
    <div className="overflow-x-hidden bg-slate-50 dark:bg-slate-900 transition-colors">
      ...
      <div className="pt-0 flex items-center gap-4 relative z-20 mb-4">
        ...
        <div>
          ...
          <p className="text-xs font-semibold text-emerald-600 uppercase tracking-widest">Global Merchant Ranking</p>
        </div>
      </div>
      ...
      <div className="">
        ...
        <div className="bg-gradient-to-br from-emerald-600 to-teal-700 dark:from-slate-900 dark:to-slate-900 p-8 rounded-2xl text-white relative overflow-hidden shadow-2xl border border-emerald-500/20 dark:border-white/5 transition-all duration-500">
          ...
          <div className="relative z-10 text-center">
            ...
            <p className="text-xs font-semibold text-slate-400 leading-relaxed max-w-[280px] mx-auto uppercase tracking-widest">
          ...
          <>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-white/5 p-5 flex items-start gap-4 mb-8">
              ...
              <div className="flex-1">
                <h4 className="text-xs font-black dark:text-white uppercase tracking-[0.2em] leading-none mb-1">Elite Merchant Status</h4>
            ...
            {topSellers.length > 3 && (
              <div className="flex flex-col gap-2 pb-24">
                <div className="flex items-center justify-between mb-2 px-2">
                  <span className="text-xs font-black uppercase tracking-widest text-slate-400">Merchant Directory</span>
                </div>
                  ...
                  >
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mt-0.5">Top Merchant</p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Revenue</p>
                    </div>
                  </div>
                ))}
              </div>
  ...
  const PodiumSlot = ({ user, height, bgClass, medalClass, isFirst }) => (
    <div className="flex flex-col items-center flex-1">
      ...
      <div className={`w-full ${user ? bgClass : 'bg-slate-100 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700'} rounded-t-2xl ${height} flex flex-col items-center justify-center relative shadow-lg`}>
        ...
        {user ? (
          <>
            <p className={`text-xs font-semibold ${isFirst ? 'text-amber-700 dark:text-amber-400' : 'text-slate-800 dark:text-white'} truncate w-20 text-center mt-2`}>{user.name}</p>
            <p className={`font-semibold ${isFirst ? 'text-lg text-amber-600' : 'text-[12px] text-primary'}`}>{user.kg} KG</p>
          </>
        ) : (
          <p className="text-xs font-semibold text-slate-300 dark:bg-slate-600 uppercase tracking-widest">Open</p>
        )}
        {isFirst && user && (
          <div className="absolute -bottom-2 bg-amber-400 text-white text-xs font-semibold px-4 py-1 rounded-full uppercase shadow-md">Hero</div>
        )}
      </div>
    </div>
  );
    ...
    <div className="overflow-x-hidden">
      ...
      <div className="pt-0 flex items-center gap-4 relative z-20">
        ...
        <div>
          ...
          <p className="text-xs font-semibold text-primary uppercase tracking-widest">Global Leaderboard</p>
        </div>
      </div>
        ...
        {isEmpty ? (
          ...
          <>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-white/5 p-5 flex items-start gap-4 mb-8">
              ...
              <div className="flex-1">
                <h4 className="text-xs font-semibold dark:text-white uppercase tracking-widest leading-none mb-1">How to Rank Up</h4>
                <p className="text-xs font-semibold text-slate-400 leading-relaxed">
            ...
            {topUsers.length > 3 && (
              <div className="flex flex-col gap-2 pb-20">
                <p className="text-xs font-semibold uppercase tracking-widest text-slate-400 px-2 mb-2">Top Recyclers List</p>
                  ...
                  >
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest">{user.kg} KG Collected</p>
                      </div>
                    </div>
