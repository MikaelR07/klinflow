...
import { useAuthStore, supabase, getThumbnailUrl } from '@cleanflow/core';
...
const PodiumSlot = memo(({ user, height, bgClass, medalClass, isFirst }) => (
  <div className="flex flex-col items-center flex-1">
    <div className={`${isFirst ? 'w-16 h-16' : 'w-12 h-12'} rounded-full border-2 ${user ? (isFirst ? 'border-amber-400 bg-amber-100' : 'border-slate-300 bg-slate-200 dark:bg-slate-700') : 'border-dashed border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-800'} flex items-center justify-center mb-2 relative overflow-hidden shadow-sm`}>
      {user?.avatarUrl ? (
        <img 
          src={getThumbnailUrl(user.avatarUrl, { width: isFirst ? 150 : 100 })} 
          className="w-full h-full object-cover" 
          alt={user.name}
          loading={isFirst ? "eager" : "lazy"}
          fetchPriority={isFirst ? "high" : "auto"}
...
const SellerListItem = memo(({ user }) => (
  <div 
  ...
  >
    <div className="flex items-center gap-5">
      ...
      <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-lg overflow-hidden border border-slate-100 dark:border-white/5">
        {user.avatarUrl ? (
          <img 
            src={getThumbnailUrl(user.avatarUrl, { width: 100 })} 
            className="w-full h-full object-cover" 
            alt={user.name}
            loading="lazy"
          />
...
const ResidentListItem = memo(({ user }) => (
  <div
...
function SellerLeaderboard() {
  ...
  return (
    <div className="overflow-x-hidden bg-slate-50 dark:bg-slate-900 transition-colors px-4 pb-20">
      {/* Header */}
...
export default function Leaderboard() {
  ...
  return (
    <div className="overflow-x-hidden px-4 pb-24 bg-[#F8F9FA] dark:bg-slate-950 transition-colors">
