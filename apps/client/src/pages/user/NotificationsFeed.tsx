...
import { ArrowLeft, BellRing, CheckCircle2, AlertTriangle, Gift, Info, Clock, MailCheck, MessageSquareCheck, CircleCheckBig } from 'lucide-react';
...
export default function NotificationsFeed() {
  ...
  return (
    <div className="pb-4 bg-white dark:bg-slate-800 ">
        ...
        <div className="flex items-center justify-between px-4 h-6">
          ...
          <div className="flex items-center gap-2">
            {notifications.length > 0 && (
              <button
                onClick={clearAll}
                className="text-[11px] font-bold capitalize tracking-widest text-white bg-red-600  px-3.5 py-1 rounded-lg transition-colors"
              >
                Clear
              </button>
            )}
      ...
      {/* ── NOTIFICATIONS FEED ── */}
      <div className="max-w-lg mx-auto pt-[calc(env(safe-area-inset-top,1rem)+4rem)] animate-slide-up space-y-2 px-1.5">
