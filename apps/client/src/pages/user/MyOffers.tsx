...
export default function MyOffers() {
  ...
  const handleRejectCounterOffer = async (b: Booking) => {
    ...
    toast.info("Trade Cancelled", {
      description: "The counter-offer was rejected.",
    });
    fetchBookings();
  };
                    ...
                    <span
                      className={`px-1 py-0.2 text-[8px] font-bold rounded ${
                        activeTab === tab.id
                          ...
                          : "bg-blue-300 dark:bg-blue-600 text-white dark:text-white"
                      }`}
                    >
                      {tab.count}
                    </span>
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar space-y-4 px-1.5 pt-[calc(env(safe-area-inset-top,1rem)+4.5rem)] pb-14">
                  ...
                  <div className="relative h-[270px] w-full overflow-hidden rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-900">
                    {selectedOffer.photo ? (
                      <OptimizedImage
                        src={getThumbnailUrl(selectedOffer.photo, {
                          width: 800,
                        })}
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs font-black text-emerald-600 leading-none">
                              KSh {selectedOffer.offeredPrice}/kg
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 block truncate max-w-[120px]">
                              {selectedOffer.listing?.location || "Nairobi Hub"}
                            </span>
                          </div>
                        </div>
                  ...
                  <div className="bg-emerald-600 dark:bg-emerald-700 p-4 rounded-2xl border border-emerald-500/20  flex flex-col justify-center text-center">
                    <p className="text-[10px] font-bold text-white dark:text-slate-200 capitalize tracking-widest mb-1">
                      Expected Value
                    </p>
                    <p className="text-xl font-black text-white dark:text-white leading-none">
                      KSh{" "}
                      {(
                        parseFloat(selectedOffer.listing?.pricePerKg || 0) *
                        parseFloat(selectedOffer.listing?.quantity || 0)
                      ).toLocaleString()}
                    </p>
                    <p className="text-[10px] text-emerald-200 dark:text-emerald-100 font-bold capitalize mt-1.5">
                      {selectedOffer.listing?.pricePerKg} ×{" "}
                      {selectedOffer.listing?.quantity}kg Total Stock
                    </p>
                  </div>

                  {/* Listing Details */}
                  <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800">
                    <h4 className="text-xs font-semibold text-slate-400 capitalize tracking-widest mb-3">
                      Bid Comparison
                    </h4>
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs font-semibold text-slate-900 dark:text-white">
                          KSh {selectedOffer.listing?.pricePerKg || "N/A"}/kg
                        </span>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {(() => {
                          const diff =
                            (selectedOffer.offeredPrice || 0) -
                            (selectedOffer.listing?.pricePerKg || 0);
                          return (
                            <span
                              className={`text-xs font-semibold ${diff >= 0 ? "text-emerald-600" : "text-rose-500"}`}
                            >
                              {diff >= 0 ? "+" : ""}
                              {diff.toFixed(2)}/kg
                            </span>
                      ...
                      <div className="flex items-center justify-between pt-1">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold text-slate-900 dark:text-white capitalize tracking-widest">
                            Your Pay
                          </span>
                          <span className="text-[10px] font-semibold text-slate-400">
                            KSh {selectedOffer.offeredPrice} ×{" "}
                            {selectedOffer.quantity}kg
                          </span>
                        </div>
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      className="flex-1 py-4 bg-red-600 text-white border border-rose-100 dark:border-rose-900/30 rounded-2xl font-semibold text-xs capitalize tracking-widest  active:scale-[0.97] transition-all"
                    >
                      Decline
                    </button>
                    <button
                      onClick={() => handleAccept(selectedOffer)}
                      className="flex-[2] py-4 bg-emerald-600 text-white rounded-2xl font-semibold text-xs capitalize tracking-[0.2em]  active:scale-[0.97] transition-all flex items-center justify-center gap-2"
                    >
                    ...
                    >
                      <div className="flex gap-3">
                        <div className="w-16 h-16 rounded-xl bg-slate-50 dark:bg-slate-800 overflow-hidden shrink-0 flex items-center justify-center text-2xl border border-slate-100 dark:border-slate-800">
                          {offer.photo ? (
                            <OptimizedImage
                              src={getThumbnailUrl(offer.photo, { width: 150 })}
                              className="w-full h-full object-cover"
                              wrapperClassName="w-full h-full"
                            />
                          ) : (
                  ...
                  inProgressOffers.map((offer) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            setExpandedTradeId(isExpanded ? null : offer.id)
                          }
                        ...
                        >
                          <div className="w-16 h-16 rounded-xl bg-slate-50 dark:bg-slate-800 overflow-hidden shrink-0 flex items-center justify-center text-2xl border border-slate-100 dark:border-slate-800">
                            {offer.photo ? (
                              <OptimizedImage
                                src={getThumbnailUrl(offer.photo, {
                                  width: 150,
                                })}
                                className="w-full h-full object-cover"
                                wrapperClassName="w-full h-full"
                            ...
                            >
                              <div className="pt-4 border-t border-slate-50 dark:border-slate-800/50 space-y-4">
                                ...
                                <div className="bg-emerald-600 dark:bg-emerald-500/10 p-5 rounded-2xl border border-emerald-500/20 shadow-xl shadow-emerald-500/10 flex items-center justify-between">
                                  <div>
                                    <p className="text-[10px] font-bold text-emerald-100 dark:text-emerald-400/60 capitalize tracking-widest mb-1">
                                      Expected Payout
                                    </p>
                                ...
                                <p className="text-[10px] font-medium text-slate-400 italic text-center px-4">
                                  Agent is currently dispatched. Funds will be
                                  released to your wallet upon weight
                                  verification.
                                </p>
                              </div>
                            </motion.div>
                  ...
                  counterOffers.map((b) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            setExpandedTradeId(isExpanded ? null : b.id)
                          }
                          className="w-full py-3 px-3.5 pl-4 flex gap-3 active:bg-slate-50 dark:active:bg-slate-800/50 transition-colors text-left"
                        >
                          ...
                          <div className="flex-1 min-w-0 flex flex-col justify-center">
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-1.5 min-w-0">
                                <h3 className="text-[11px] font-bold text-slate-900 dark:text-white capitalize truncate tracking-tight">
                                  {b.wasteType || "Materials"}
                                </h3>
                                <span className="px-1 py-0.5 rounded text-[8px] font-bold bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400 flex items-center gap-0.5 shrink-0">
                                  PRICE REVISED
                                </span>
                              </div>
                              <span className="text-sm font-black text-amber-600 tracking-tighter shrink-0 ml-2">
                                KSh{" "}
                                {(
                                  (b as any).counterOfferAmount || 0
                                ).toLocaleString()}
                              </span>
                            </div>
                              ...
                              <div className="pt-4 border-t border-slate-50 dark:border-slate-800/50 space-y-4">
                                <div className="bg-amber-500/5 dark:bg-amber-500/10 p-4 rounded-2xl border border-amber-500/20">
                                  <div className="flex justify-between items-center mb-3">
                                    <p className="text-[10px] font-black text-amber-600 capitalize tracking-widest">
                                      Revision Details
                                    </p>
                                    <p className="text-[10px] font-black text-slate-400 capitalize tracking-widest">
                                      Agent: {(b as any).buyerName || "Agent"}
                                    </p>
                                  </div>
                                  <div className="flex justify-between items-end">
                                    <div>
                                      <p className="text-[10px] font-semibold text-slate-400 capitalize tracking-widest mb-0.5">
                                        Original Payout
                                      </p>
                                    ...
                                    <div className="text-right">
                                      <p className="text-[10px] font-semibold text-amber-600 capitalize tracking-widest mb-0.5">
                                        Counter-Offer
                                      </p>
                                ...
                                <p className="text-xs font-medium text-slate-400 italic text-center px-4">
                                  Accepting this revision will finalize the
                                  trade at the new price.
                                </p>
                              </div>
                            </motion.div>
                          )}
                ...
                {completedOffers.length === 0 ? (
                  ...
                  completedOffers.map((offer) => {
                    ...
                    return (
                      <div
                        key={offer.id}
                        className="bg-white dark:bg-slate-800 border-y border-slate-100 dark:border-slate-800 -mx-1 transition-all relative overflow-hidden"
                      >
                        ...
                        >
                          ...
                          <div className="flex-1 text-left min-w-0">
                            <div className="flex items-center justify-between mb-0.5">
                              <h4 className="text-xs font-semibold text-slate-900 dark:text-white truncate capitalize tracking-tight">
                                {offer.material}
                              </h4>
                              <span className="text-[10px] font-semibold bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 px-2 py-0.5 rounded capitalize">
                                Settled
                              </span>
                            </div>
                            <p className="text-[10px] text-slate-400 font-semibold capitalize tracking-widest truncate">
                              Agent: {offer.buyerName}
                            </p>
                          </div>
                          <ChevronDown
                            className={`w-4 h-4 text-slate-300 transition-transform duration-300 ${isExpanded ? "rotate-180" : ""}`}
                            ...
                            >
                              <div className="pt-4 border-t border-slate-50 dark:border-slate-800/50 space-y-4">
                                ...
                                <div className="bg-emerald-600 p-4 rounded-2xl flex items-center justify-between shadow-lg shadow-emerald-600/20">
                                  <div>
                                    <p className="text-[10px] font-semibold text-white/70 capitalize tracking-widest mb-0.5">
                                      Total Payout
                                    </p>
                                ...
                                <div className="flex items-center gap-2 text-[10px] font-semibold text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 p-3 rounded-xl justify-center">
                                  ...
                                  <span className="capitalize tracking-widest">
                                    Verified & Transferred to Wallet
                                  </span>
                                </div>
                              </div>
                            </motion.div>
