...
const LOAN_OFFERS: any[] = [
  {
    ...
    description: 'Triple your collection capacity with a heavy-duty motorbike.',
    amount: 45000,
    tenure: '12 Months',
    interest: '5% Fixed',
    requiredScore: 75,
...
export default function FinancingHub() {
  ...
  const [selectedLoan, setSelectedLoan] = useState<any>(null); return (
    <div className="min-h-screen bg-[#F8F8FF] dark:bg-slate-800 transition-colors">
      ...
      <div className="w-full pt-[calc(env(safe-area-inset-top,1rem)+4.5rem)] pb-24 px-1.5 space-y-6 max-w-lg mx-auto">
        {/* ── REPUTATION SNAPSHOT ── */}
        <div className="bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 text-slate-900 dark:text-white shadow-sm relative overflow-hidden">
            ...
            {LOAN_OFFERS.map((loan: any) => {
              ...
              return (
                <div
                  ...
                  className={`p-5 rounded-2xl border-2 transition-all relative overflow-hidden group ${isUnlocked
                    ? 'bg-white dark:bg-slate-800 border-slate-800 dark:bg-slate-900/50 active:scale-[0.98]'
