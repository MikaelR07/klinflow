...
export default function PostTrade() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F8F8FF] dark:bg-slate-900 transition-colors">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl pt-[calc(env(safe-area-inset-top,1rem)+0.75rem)] pb-3 px-4 border-b border-slate-200 dark:border-slate-800 max-w-lg mx-auto">
                    ...
                    <div className="flex flex-col">
                      ...
                      {drillDownCompany && (
                        <div className="flex items-center gap-1 mt-0.5">
                           ...
                           <span className="text-xs font-bold text-slate-500 uppercase">{drillDownCompany.companyName || drillDownCompany.name}</span>
                        </div>
                      )}
                    </div>
...
<Popup className="compact-popup">
                                 <div className="p-3 text-center">
                                   ...
                                   <p className="text-xs text-slate-500 mt-1 uppercase tracking-widest">{hub.hubAddress}</p>
                              ...
                              <Marker key={weaver.id} position={[weaver.location?.latitude || (center[0] + 0.005), weaver.location?.longitude || (center[1] + 0.005)] as [number, number]} {...({ icon: weaverIcon } as any)}>
                                ...
                                <Popup className="compact-popup">
                                  <div className="p-2 text-center">
                                    <h4 className="text-xs font-semibold text-indigo-600 uppercase tracking-tight">{weaver.businessName || 'Collector'}</h4>
                    ...
                    {selectedHub ? (
                      <div className="bg-emerald-50 dark:bg-emerald-900/20 p-6 rounded-2xl border border-emerald-100 dark:border-emerald-900/30 flex items-center gap-4">
                         ...
                         <div className="flex-1">
                            ...
                            <p className="text-xs font-semibold text-emerald-600 uppercase tracking-widest mt-1">Drop-off: {selectedHub.hubAddress} ({selectedHub.distance.toFixed(1)}km)🏢</p>
                         </div>
