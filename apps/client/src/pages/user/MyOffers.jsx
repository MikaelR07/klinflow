...
export default function MyOffers() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F8F8FF] dark:bg-slate-900 transition-colors">
      ...
      <div className="-mx-1 -mt-[calc(env(safe-area-inset-top,1.5rem)+1.5rem)] bg-white dark:bg-slate-900 pt-[calc(env(safe-area-inset-top,1.5rem)+0.75rem)] pb-0 px-4 border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center justify-between max-w-lg mx-auto pb-4">
          ...
          <div className="text-center">
            <h1 className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-tighter leading-none">My Offers</h1>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mt-1">Marketplace</p>
          </div>
              ...
              >
                ...
                {tab.count > 0 && (
                  <span className={`px-1.5 py-0.5 text-[9px] font-bold rounded-md ${
                    activeTab === tab.id ? 'bg-emerald-600 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-500'
                  }`}>
                    {tab.count}
                  </span>
                )}
      ...
      {/* ── CONTENT AREA ── */}
      <div className={`px-3 pt-4 ${selectedOfferId ? "animate-fade-in" : ""}`}>
