...
export default function EditSwarm() {
  ...
  const updateSwarm = useCollectiveStore(s => s.updateSwarm);
  ...
  useEffect(() => {
    ...
    if (id) {
      fetchSwarmById(id).then(res => {
        if (res.swarm) {
          setMaterial(res.swarm.material);
          setTargetWeight(res.swarm.target_weight.toString());
          setDescription(res.swarm.description || '');
          if (res.swarm.images) {
            setPhotos(res.swarm.images.map(url => ({ preview: url, file: null, isExisting: true })));
          }
        }
      });
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const uploadPromises = photos.map(async (p, i) => {
        if (p.isExisting) return p.preview;
      ...
      const { success } = await updateSwarm(id!, {
        material,
      ...
      if (success) {
        ...
        toast.error('Failed to update swarm. Try again.');
      }
    } catch (error) {
      console.error(error);
      toast.error('Failed to upload images or update swarm');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="fixed top-0 left-0 right-0 z-50 max-w-lg mx-auto bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl border-b border-slate-200 dark:border-slate-600/60 transition-all duration-300">
        <div className="pt-[calc(env(safe-area-inset-top,1rem)+1rem)] pb-3.5 px-4 flex items-center justify-between">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-900 dark:text-white tracking-tighter leading-tight">Edit Swarm</h1>
