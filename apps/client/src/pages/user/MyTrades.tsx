...
import {
  ...
  Info, ChevronDown, ChevronUp, Tag, Coins,Scale,MapPin, ArrowLeft, Trash2
} from 'lucide-react';
...
const TradeCounter = ({ count, active }: { count: number, active: boolean }) => (
  <span className={`ml-1.5 px-2 py-0.5 text-xs font-semibold rounded-full transition-all ${active
        ...
        <div className="max-w-lg mx-auto space-y-2.5">
          <div className="flex items-center justify-between">
            ...
            <div className="text-center">
              <h1 className="text-base font-bold text-slate-900 dark:text-white capitalize tracking-widest leading-none">Trade History</h1>
              <p className="text-[9px] font-bold text-slate-500 capitalize tracking-[0.2em] mt-0.5">View All Your Trades Activities </p>
            </div>
            ...
            {TABS.map(tab => {
              ...
              return (
                ...
                >
                  ...
                  {tabCount > 0 && (
                    <span className={`px-1 py-0.2 text-[8px] font-bold rounded ${activeTab === tab ? 'bg-emerald-600 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-300'
                      }`}>
                      {tabCount}
                    </span>
                  )}
                        ...
                        <div className="pt-[calc(env(safe-area-inset-top,1rem)+0.75rem)] pb-3.5 px-4 flex items-center gap-3.5">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] font-bold text-primary capitalize tracking-widest flex items-center gap-1.5 mt-0.5">
                              <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" /> Marketplace Trade
                            </p>
                          </div>
                        </div>
                      </div>
                          ...
                          {(() => {
                            ...
                            return photoUrl ? (
                              ...
                              <div className="w-full h-full flex flex-col items-center justify-center bg-slate-100 dark:bg-slate-800 text-slate-400">
                                <div className="text-5xl mb-3">{waste?.icon || '♻️'}</div>
                                <p className="text-[10px] font-bold tracking-widest uppercase">No photo attached</p>
                              </div>
                            );
                          })()}
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-100 dark:border-slate-800/40 space-y-4 shadow-sm">
                          <div className="flex items-start justify-between">
                            ...
                            <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md ${status.color} border border-current/20 bg-current/10`}>
                              ...
                              <span className="text-[9px] font-bold uppercase tracking-wider leading-none mt-px">{status.label}</span>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Trade ID</p>
                                <p className="text-xs font-black text-slate-900 dark:text-white font-mono uppercase">CF-{b.id?.slice(0, 8)}</p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Trade Value</p>
                                <p className="text-xs font-black text-emerald-600 leading-none">KSh {(b.totalPrice || b.amount || 0).toLocaleString()}</p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs font-black text-slate-900 dark:text-white capitalize">{b.actualWeightKg || b.bags || 0} KG</p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 block truncate max-w-[120px]">{b.estate || 'Pickup point'}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                ...
                {filteredBookings.length === 0 ? (
                  ...
                  <div className="space-y-0">
                    {filteredBookings.map((b: any) => {
                            ...
                            >
                              <div className="w-16 h-16 rounded-xl bg-slate-50 dark:bg-slate-800 overflow-hidden shrink-0 flex items-center justify-center text-2xl border border-slate-100 dark:border-slate-800">
                                {b.photoUrl ? (
                                  ...
                                  <div className="w-full h-full bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center">{waste?.icon || '♻️'}</div>
                                )}
                              </div>
                              <div className="flex-1 min-w-0 flex flex-col justify-center">
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="text-sm font-black text-emerald-600 dark:text-emerald-400 tracking-tighter shrink-0 ml-2">KSh {(b.totalPrice || b.amount || 0).toLocaleString()}</span>
                                </div>
                                ...
                                <div className="flex items-center justify-between mt-0.5">
                                  <p className="text-[10px] font-bold text-slate-400 flex items-center gap-1 capitalize truncate max-w-[150px]">
                                    <Tag className="w-2.5 h-2.5 text-indigo-500" /> CF-{b.id?.slice(0, 8)}
                                  </p>
                                </div>
                                ...
                                <div className="flex items-center justify-between pt-1 mt-1 border-t border-slate-50 dark:border-slate-800/50">
                                  <p className="text-[10px] font-bold text-slate-400 flex items-center gap-1 capitalize shrink-0">
                                    <Clock className="w-2.5 h-2.5 text-slate-400" /> {b.updatedAt || b.createdAt ? new Date(b.updatedAt || b.createdAt).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'Pending'}
                                  </p>
                                  <p className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 flex items-center gap-1 capitalize shrink-0">
                                    ...
                                    <Scale className="w-2.5 h-2.5" /> {b.actualWeightKg || b.bags || 0} KG
                                  </p>
                                </div>
                              </div>
                                  ...
                                  <div className="px-4 pb-4 pt-1 border-t border-slate-100 dark:border-slate-800 space-y-2">
                                      ...
                                      <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div className="flex items-start gap-3">
                                          ...
                                          <div>
                                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Quantity</p>
                                            <p className="text-xs font-black text-slate-900 dark:text-white">{b.actualWeightKg || b.bags || 0} KG</p>
                                          </div>
                                        </div>
                                        ...
                                        <div className="flex items-start gap-3">
                                          <Coins className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
                                          <div>
                                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Settled Amount</p>
                                            <p className="text-xs font-black text-emerald-600">KSh {(b.totalPrice || b.amount || 0).toLocaleString()}</p>
                                          </div>
                                        </div>
                                        ...
                                        <div className="flex items-start gap-3">
                                          ...
                                          <div>
                                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Completed</p>
                      ...
                      return (
                        <div key={b.id} className="bg-white dark:bg-slate-900/60 py-3 px-3.5 shadow-sm border-b border-slate-100 dark:border-slate-700 active:bg-slate-50 dark:active:bg-slate-800/50 transition-colors cursor-pointer">
                          <button
                          ...
                          >
                            <div className="w-16 h-16 rounded-xl bg-slate-50 dark:bg-slate-800 overflow-hidden shrink-0 flex items-center justify-center text-2xl border border-slate-100 dark:border-slate-800">
                              {b.photoUrl ? (
                                ...
                                <div className="w-full h-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center">{waste?.icon || '♻️'}</div>
                              )}
                            </div>
                            <div className="flex-1 min-w-0 flex flex-col justify-center">
                              {/* Row 1: Material & Price */}
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-1.5 min-w-0">
                                  ...
                                  <span className={`px-1 py-0.5 rounded text-[8px] font-bold ${status.color} flex items-center gap-0.5 shrink-0`}>
                                    {status.label}
                                  </span>
                                </div>
                                <span className="text-sm font-black text-emerald-600 dark:text-emerald-400 tracking-tighter shrink-0 ml-2">KSh {(b.totalPrice || b.amount || 0).toLocaleString()}</span>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {TIME_SLOTS.map(slot => (
                          <button
                            ...
                            className={`py-3 px-1 rounded-xl text-[11px] font-bold capitalize tracking-tight transition-all border ${newTime === slot.value
