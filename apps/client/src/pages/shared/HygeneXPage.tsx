...
import { Brain, Mic, Send, Lightbulb, MapPin, StopCircle, ShieldCheck, Activity, User, ArrowLeft, BrainCircuit } from 'lucide-react';
...
function useVoiceRecognition() {
  ...
  const startListening = (onResult: (text: string) => void) => {
    ...
    if (!SpeechRecognition) {
      ...
      setTimeout(() => {
        onResult("What are the current market prices?");
        setIsListening(false);
        isListeningRef.current = false;
      }, 2000);
      return;
...
export default function HygeneXPage() {
  ...
  const renderMessageText = (text: string) => {
    ...
    cleanText = cleanText.replace(/cleanflow/gi, 'Klinflow').replace(/CleanFlow/g, 'Klinflow');
    return cleanText;
  };
  ...
  return (
    <div className="flex flex-col fixed inset-0 bg-gradient-to-br from-teal-600 to-emerald-600 dark:from-teal-900 dark:to-emerald-950 text-white z-50 overflow-hidden">
      {/* Pattern Overlay */}
      ...
      {/* ── FIXED TOP NAV ── */}
      <div className="fixed top-0 left-0 right-0 bg-gradient-to-br from-teal-600 to-emerald-600 dark:bg-slate-950/70 backdrop-blur-xl pt-[calc(env(safe-area-inset-top,1rem)+1rem)] pb-4 px-4 border-b border-slate-200/50 dark:border-white/5 z-50 transition-colors">
          ...
          <div className="max-w-3xl mx-auto space-y-10">
            {messages.length <= 1 && !isTyping ? (
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center justify-center pt-8 pb-12">
                ...
                <p className="text-sm text-slate-100 dark:text-slate-400 mb-10 text-center max-w-sm">
                ...
                <div className="grid grid-cols-2 gap-3 w-full max-w-lg px-2">
                  {getSuggestions().map((cap, idx) => (
                    <button
                      key={idx}
                      onClick={() => sendMessage(cap.title)}
                      className="p-4 rounded-[1.5rem] bg-white/80 dark:bg-slate-900/60 backdrop-blur-md border border-white/80 dark:border-white/5 text-left transition-all active:scale-95 hover:shadow-lg hover:-translate-y-1 hover:bg-white/80 dark:hover:bg-slate-900/80 flex flex-col items-start gap-3 shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-[0_4px_20px_rgb(0,0,0,0.2)]"
                    >
