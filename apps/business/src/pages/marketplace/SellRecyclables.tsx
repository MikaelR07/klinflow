      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="glass p-6 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-4">
            ...
            {/* Material */}
            <div>
              <label className="block text-xs font-semibold text-slate-800 uppercase tracking-wider mb-1.5">Material Type</label>
              <div className="relative">
                <Tag className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-700" />
                <select 
                  name="material" 
            ...
            <div className="grid grid-cols-2 gap-4">
               <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-widest mb-1.5">Material Grade</label>
                <select 
                  name="grade" 
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-widest mb-1.5">Listing Unit</label>
                <div className="flex bg-white shadow-sm dark:bg-slate-800 p-1 rounded-xl">
                  {['KG', 'Tons'].map(u => (
                    <button
                      key={u}
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, unit: u }))}
                      className={`flex-1 py-2 text-xs font-semibold rounded-lg transition-all ${formData.unit === u ? 'bg-white dark:bg-slate-700 text-primary shadow-sm' : 'text-slate-800'}`}
                    >
                      {u}
                    </button>
                  ))}
            ...
            <div className="grid grid-cols-2 gap-4">
              {/* Quantity */}
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-widest mb-1.5">Total Quantity ({formData.unit})</label>
                <div className="relative">
                  <Scale className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-700" />
                  <input 
                    type="number" 
                    name="quantity" 
                    placeholder="e.g. 50"
              ...
              {/* MOQ */}
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-widest mb-1.5">Min. Order (MOQ)</label>
                <div className="relative">
                  <Package className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-700" />
                  <input 
                    type="number" 
                    name="moq" 
                    placeholder="e.g. 1"
            ...
            {/* Price */}
            <div>
              <label className="block text-xs font-semibold text-slate-700 uppercase tracking-widest mb-1.5">Price / {formData.unit} (KES)</label>
              <div className="relative">
                <Coins className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-700" />
                <input 
                  type="number" 
                  name="pricePerKg" 
            ...
            {/* Description */}
            <div>
              <label className="block text-xs font-semibold text-slate-800 uppercase tracking-wider mb-1.5">Description (Optional)</label>
              <div className="relative">
                <Info className="absolute left-4 top-3 h-5 w-5 text-slate-700" />
                <textarea 
                  name="description" 
            ...
            {/* Photo Upload */}
            <div>
              <label className="block text-xs font-semibold text-slate-800 uppercase tracking-wider mb-1.5">Listing Photo</label>
              <label className="border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl p-8 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-primary transition-all group">
                <Upload className="w-8 h-8 text-slate-700 group-hover:text-primary transition-colors" />
                <span className="text-xs font-medium text-slate-800">{photoFile ? photoFile.name : 'Tap to upload material photo'}</span>
                <input 
                  type="file" 
                  accept="image/*" 
                  className="hidden" 
        ...
        <div className="space-y-4">
          <h2 className="text-sm font-semibold text-slate-800 uppercase tracking-widest pl-2">Live Preview</h2>
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-primary/30 overflow-hidden shadow-lg transform scale-[0.98] opacity-90 transition-all">
            <div className="aspect-video bg-white shadow-sm dark:bg-slate-800 relative">
            ...
            <div className="p-4">
              ...
              <p className="text-xs text-slate-800 flex items-center gap-1 mb-2">
                <MapPin className="w-3 h-3 text-primary" /> {profile.location?.estate || 'Your Location'}
              </p>
                <div className="flex justify-between items-end">
                  <div className="text-right">
                    <div className="text-xs text-slate-700 font-semibold uppercase tracking-tighter">Grade</div>
                    <div className="text-xs font-semibold text-slate-900 dark:text-white uppercase">{formData.grade || 'Mix'}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-lg font-semibold text-primary leading-none">KES {formData.pricePerKg || '0'}</div>
                    <div className="text-xs text-slate-700 font-semibold uppercase tracking-tighter">per {formData.unit}</div>
                  </div>
                </div>

                <div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center text-xs font-semibold uppercase text-slate-800">
