      ...
      <div className="bg-[#F4F4F4]/80 dark:bg-slate-900/80 backdrop-blur-xl pt-2 pb-2 px-0 relative z-10 border-b border-slate-200 dark:border-slate-800">
        ...
        <div className="px-3 py-1 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">42 Online</span>
          </div>
          <div className="flex items-center gap-1.5 px-2 py-0.5 bg-emerald-500/10 rounded-md">
            <TrendingUp className="w-3 h-3 text-emerald-500" />
            <span className="text-[9px] font-black text-emerald-600 uppercase">Market Active</span>
          </div>
        </div>
      </div>
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex flex-col ${msg.isMe ? 'items-end' : 'items-start'}`}>
              <div className="flex items-center gap-2 mb-1">
                {!msg.isMe && <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">{msg.user}</span>}
