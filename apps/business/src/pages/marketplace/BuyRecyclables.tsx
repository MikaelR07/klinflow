    ...
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      <div className="flex items-center gap-3 px-1">
        ...
        <div>
          ...
          <p className="text-xs font-semibold text-slate-700 uppercase tracking-widest">{filteredListings.length} active listings</p>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-700" />
          <input
            type="text"
        ...
        <button
          ...
          className={`p-3.5 rounded-2xl border transition-all ${showFilters ? 'bg-primary border-primary text-white' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-800'}`}
        >
      ...
      {showFilters && (
        <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[0.45rem] shadow-sm animate-in slide-in-from-top-4 duration-300">
          <p className="text-xs font-bold text-slate-700 uppercase tracking-widest mb-4">Filter by Category</p>
          <div className="flex flex-wrap gap-2">
            {['All', 'Plastic', 'Paper', 'Metal', 'Glass', 'Organic', 'E-Waste'].map(mat => (
              <button
                key={mat}
                onClick={() => setSelectedMaterial(mat)}
                className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${selectedMaterial === mat ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-white shadow-sm dark:bg-slate-800 text-slate-900 dark:text-slate-400'}`}
              >
                {mat}
              </button>
            ))}
      ...
      <div className="space-y-5">
        {isLoading && listings.length === 0 ? (
          <div className="py-20 text-center">
            ...
            <p className="text-sm text-slate-800 font-medium">Scanning Marketplace...</p>
          </div>
          ...
          filteredListings.map((item) => (
            <div key={item.id} className="bg-white dark:bg-slate-900 rounded-[0.45rem] border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-xl hover:border-primary/30 transition-all group">
              <div className="relative aspect-[16/9] overflow-hidden bg-white shadow-sm dark:bg-slate-800">
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-700 font-bold uppercase tracking-widest mt-2">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-xs text-slate-700 font-bold uppercase tracking-widest">per {item.unit}</span>
                  </div>
                </div>

                <div className="flex items-center gap-3 mb-4">
                  {item.grade && (
                    <div className="px-3 py-1 bg-white shadow-sm dark:bg-slate-800 rounded-lg flex items-center gap-2">
                      <span className="text-xs font-bold text-slate-700 uppercase tracking-widest leading-none pt-0.5">Grade</span>
                      <span className="text-xs font-black text-slate-900 dark:text-slate-200 uppercase tracking-widest leading-none pt-0.5">{item.grade}</span>
                    </div>
                  )}
                ...
                <div className="flex items-center gap-4 py-4 border-y border-slate-50 dark:border-slate-800/50 my-4 text-center">
                  <div className="flex-1 border-r border-slate-50 dark:border-slate-800/50">
                    <div className="text-xs text-slate-700 font-bold uppercase tracking-widest mb-1">Available</div>
                    <div className="text-sm font-bold text-slate-900 dark:text-white">{item.quantity} {item.unit}</div>
                  </div>
                  <div className="flex-1 border-r border-slate-50 dark:border-slate-800/50">
                    <div className="text-xs text-slate-700 font-bold uppercase tracking-widest mb-1">Min. MOQ</div>
                    <div className="text-sm font-bold text-slate-900 dark:text-white">{item.moq} {item.unit}</div>
                  </div>
                  <div className="flex-1">
                    <div className="text-xs text-slate-700 font-bold uppercase tracking-widest mb-1">Lot Value</div>
                    <div className="text-sm font-bold text-slate-900 dark:text-white">KES {(item.quantity * item.pricePerKg).toLocaleString()}</div>
                  </div>
                </div>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button
                    ...
                    className="flex-1 py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-300 rounded-2xl font-bold uppercase tracking-widest text-[11px] active:scale-[0.98] transition-all"
                  >
                    Negotiate
                  </button>
                </div>
          ...
          <div className="text-center py-20 bg-slate-50 dark:bg-slate-900/50 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-800 mt-1 uppercase tracking-widest">Try adjusting your radar</p>
          </div>
        )}
      </div>
      ...
      {detailModal && (
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-4 bg-slate-900/80 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-[3rem] p-8 shadow-2xl border border-white/10 overflow-hidden relative animate-in slide-in-from-bottom-12 duration-500">
            <button onClick={() => setDetailModal(null)} className="absolute top-6 right-6 p-2 rounded-full bg-white shadow-sm dark:bg-slate-800 text-slate-800">
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="mb-8">
              ...
              <p className="text-xs font-bold text-slate-700 uppercase tracking-widest">Official Industrial Certification</p>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div className="bg-slate-50 dark:bg-slate-900/50 rounded-3xl p-6 border border-slate-100 dark:border-slate-800">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-black text-slate-800 uppercase tracking-widest">HygeneX Authenticated</span>
                  </div>
                  <span className="text-xs font-mono font-bold text-indigo-600">ID: BALE-{detailModal.id?.slice(0,6).toUpperCase()}</span>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs font-bold text-slate-700 uppercase tracking-widest mb-1">Purity Score</p>
                    <p className="text-2xl font-black text-slate-900 dark:text-white">98.4%</p>
                  </div>
                  <div>
                    <p className="text-xs font-bold text-slate-700 uppercase tracking-widest mb-1">Moisture Level</p>
              ...
              {/* Provenance Timeline */}
              <div>
                <h4 className="text-xs font-black text-slate-700 uppercase tracking-[0.2em] mb-6 px-1">Market Provenance</h4>
                <div className="space-y-6 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-white shadow-sm dark:before:bg-slate-800">
                  {[
                    { label: 'Market Listing', desc: 'Active on Klinflow Terminal', date: 'Today', icon: ShoppingCart, color: 'bg-indigo-500' },
                    ...
                    <div key={i} className="flex gap-4 relative">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs font-bold text-slate-700 uppercase">{step.date}</span>
                        </div>
                        <p className="text-xs text-slate-800 mt-0.5">{step.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-[3rem] p-8 shadow-2xl border border-slate-200 dark:border-slate-800 animate-in slide-in-from-bottom-12 duration-500">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-xs font-bold text-slate-700 uppercase tracking-widest mt-1">Escrow Protected Transaction</p>
              </div>
              <button onClick={() => setOrderModal(null)} className="p-2 rounded-full hover:bg-white shadow-sm dark:hover:bg-slate-800 text-slate-700">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-6">
              <div>
                <div className="flex justify-between items-end mb-2">
                  <label className="text-xs font-bold text-slate-700 uppercase tracking-widest">Order Quantity</label>
                ...
                <div className="relative">
                  ...
                  />
                  <span className="absolute right-6 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-700 uppercase">{orderModal.unit}</span>
                </div>
              </div>
              ...
              {orderQty && Number(orderQty) > 0 && (
                <div className="flex justify-between items-center p-5 bg-indigo-50 dark:bg-indigo-500/5 border border-indigo-100 dark:border-indigo-500/20 rounded-[2rem] animate-in zoom-in-95 duration-200">
                  <span className="text-xs font-bold text-slate-800 uppercase tracking-widest">Total Settle</span>
              ...
              <p className="text-center text-xs text-slate-700 font-bold uppercase tracking-widest">Funds held securely until delivery verified</p>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-[2rem] p-6 shadow-2xl border border-slate-200 dark:border-slate-800 animate-in slide-in-from-bottom-8 duration-300">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-xs text-slate-800">Negotiate for {offerModal.material}</p>
              </div>
              <button onClick={() => setOfferModal(null)} className="p-2 rounded-full hover:bg-white shadow-sm dark:hover:bg-slate-800 text-slate-700">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-700">KES</span>
                </div>
              </div>
