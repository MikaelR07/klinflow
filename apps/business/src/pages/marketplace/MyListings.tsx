...
export default function MyListings() {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      {/* Tabs */}
      <div className="flex bg-white shadow-sm dark:bg-slate-900 p-1.5 rounded-2xl">
        {stats.map((tab) => (
          <button
            key={tab.id}
            ...
            className={`flex-1 py-2.5 rounded-xl text-xs font-semibold transition-all ${
              activeTab === tab.id 
                ...
                : 'text-slate-800 hover:text-slate-900 dark:hover:text-slate-600'
            }`}
          >
      ...
      <div className="space-y-4">
        {filteredListings.length > 0 ? (
          filteredListings.map((item) => (
            <div key={item.id} className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm relative group transition-all hover:border-primary/20">
              <div className="flex gap-4">
                <img src={getThumbnailUrl(item.photo, { width: 200 })} loading="lazy" alt={item.material} className="w-20 h-20 rounded-2xl object-cover bg-white shadow-sm dark:bg-slate-800" />
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <h3 className="font-semibold text-slate-900 dark:text-white">{item.material} Batch</h3>
                    <button className="p-1 -mr-1 text-slate-700 hover:text-slate-900 dark:hover:text-slate-600">
                      <MoreVertical className="w-4 h-4" />
                    </button>
                  </div>
                  <p className="text-xs font-semibold text-slate-700 uppercase tracking-widest mb-2">
                    Posted on {new Date(item.createdAt).toLocaleDateString()}
                  </p>
                  
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-1 text-xs font-semibold text-slate-900 dark:text-slate-400">
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                <div className="flex items-center gap-1.5">
                   ...
                   <span className="text-xs font-semibold uppercase tracking-tighter text-slate-800">
                     Status: {item.status}
                   </span>
                </div>
          ...
          <div className="text-center py-20 bg-slate-50 dark:bg-slate-900/50 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800">
            ...
            <p className="text-sm text-slate-800 mt-1 mb-6">You haven't {activeTab === 'active' ? 'posted' : activeTab} any listings yet.</p>
            {activeTab === 'active' && (
              <button 
