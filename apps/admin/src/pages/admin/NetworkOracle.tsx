...
const getMaterialIcon = (label: string | null | undefined) => {
  ...
  if (s.includes('plastic') || s.includes('pet')) return '🧃';
  if (s.includes('metal') || s.includes('iron') || s.includes('aluminium')) return '🥫';
  if (s.includes('paper') || s.includes('cardboard')) return '📄';
  if (s.includes('glass')) return '🍾';
  if (s.includes('ewaste') || s.includes('electronic') || s.includes('e-waste')) return '💻';
...
export default function NetworkOracle() {
  ...
  const totalMarketValue = assets.reduce((sum, a: any) => sum + (a.estimated_value || a.market_value || 0), 0);
  const matchedAssets = assets.filter((a: any) => a.status === 'matched').length;
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-900 flex items-center justify-center text-xl shadow-sm">
                        {getMaterialIcon(m.label)}
                      </div>
                      <span className="font-semibold text-sm text-slate-700 dark:text-slate-200">{m.label}</span>
                    </div>
                    {editingId === m.id ? (
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {assets.slice(0, 5).map((a: any) => (
                    <tr key={a.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-8 py-4">
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-xs">
                            {getMaterialIcon(a.material_type)}
                          </div>
                          <span className="text-xs font-semibold text-slate-800 dark:text-slate-200 uppercase">{a.material_type}</span>
                        </div>
                      </td>
                      <td className="px-8 py-4 text-xs font-semibold text-slate-700 dark:text-slate-300">{a.weight_kg || a.weight || 0} KG</td>
                      <td className="px-8 py-4">
                      ...
                      <td className="px-8 py-4 font-semibold text-xs text-indigo-600 dark:text-indigo-400">KSh {a.estimated_value?.toLocaleString() || a.market_value?.toLocaleString() || 0}</td>
