...
import { supabase, usePriceStore, useServiceStore, useAgentStore, useAuthStore, WASTE_CATEGORIES, getCategoryBySlug } from '@cleanflow/core';
...
export default function AIScannerModal({ isOpen, onClose, onVerify, booking }) {
  ...
  const customCatalog = profile?.service_profile?.custom_services || [];
  const hasCustomCatalog = customCatalog.length > 0;

  // Map custom catalog to standard format for the UI
  const uiCategories = hasCustomCatalog ? customCatalog.map(cat => ({
  ...
  const currentCategory = uiCategories.find(c => c.id === material) || uiCategories[0];
  ...
  const getRate = () => {
    if (hasCustomCatalog) {
      const cat = uiCategories.find(c => c.id === material);
  ...
  const [customPayoutAmount, setCustomPayoutAmount] = useState(null);
  ...
  const estimatedGFP = isMarketTrade ? 0 : Math.floor(weight * 2);
                    ...
                    <div className="flex-1 space-y-1">
                      ...
                      <select 
                        ...
                        onChange={(e) => {
                          ...
                          if (catData?.subcategories?.length) setSubcategory(catData.subcategories[0].id);
                        }}
