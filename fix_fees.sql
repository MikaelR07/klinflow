...
BEGIN
    ...
    IF p_client_uuid IS NOT NULL THEN
        ...
        IF v_client_gfp > 0 THEN
            ...
            VALUES (p_client_uuid, 'earn', v_client_gfp, COALESCE(v_wallet_points_before, 0), COALESCE(v_wallet_points_before, 0) + v_client_gfp, 'Earned points from pickup');
        END IF;
...
BEGIN
    ...
    -- PAY SELLER (NET + 5 GFP)
    INSERT INTO public.user_wallets (user_id, cash_balance, lifetime_cash_earned, available_points, lifetime_earned)
    VALUES (v_client_id, v_net_payout, v_net_payout, 5, 5)
    ON CONFLICT (user_id) DO UPDATE SET 
        ...
        available_points = user_wallets.available_points + 5,
        lifetime_earned = user_wallets.lifetime_earned + 5;

    -- GFP LEDGER
    INSERT INTO public.wallet_ledger (user_id, transaction_type, amount, balance_before, balance_after, description)
    VALUES (v_client_id, 'earn', 5, COALESCE((SELECT available_points - 5 FROM public.user_wallets WHERE user_id = v_client_id), 0), COALESCE((SELECT available_points FROM public.user_wallets WHERE user_id = v_client_id), 5), 'Earned 5 GFP from marketplace trade');
    ...
    RETURN jsonb_build_object('status', 'success', 'gfp_earned', 5);
END;
$$;
...
BEGIN
    ...
    -- Fix bug: Check buyer balance and deduct
