...
BEGIN
    ...
    IF v_seller_id IS NOT NULL THEN
        ...
        VALUES (v_seller_id, v_net_payout, v_net_payout, v_gfp, v_gfp)
        ...
        INSERT INTO public.wallet_transactions (profile_id, amount, transaction_type, reference_id, metadata)
        VALUES (v_seller_id, v_net_payout, 'payout', p_order_id, jsonb_build_object('role', 'seller', 'type', 'marketplace_dropoff'));
        
        INSERT INTO public.notifications (target_user, target_role, type, title, body)
        VALUES (
            v_seller_id, 'all', 'success', 'Payment Received! 💰', 
            'Your drop-off has been processed. You earned KSh ' || ROUND(v_net_payout, 2) || ' and ' || v_gfp || ' GFP.'
        );
    END IF;

    -- 3. ADD TO PLATFORM TREASURY
    INSERT INTO public.platform_treasury (source_type, amount, reference_id)
    VALUES ('marketplace_dropoff_fee', v_platform_cut, p_order_id);
    ...
    RETURN jsonb_build_object('success', true, 'message', 'Payment processed successfully with platform fee');
END;
$$;
