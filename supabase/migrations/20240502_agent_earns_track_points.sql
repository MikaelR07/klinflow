-- Migration: 20240502_agent_earns_track_points.sql
-- Description: Updates the complete_booking_split_payout RPC to also award GFP (Track Points) to the agent/company executing the pickup.
...
BEGIN
    ...
    -- Pay Agent / Company Wallet (100% to Wallet, 0% to Held) AND Award Track Points
    UPDATE profiles 
    ...
    -- so their personal hero card reflects their work, even if the money went to the company
    IF v_account_type = 'fleet_driver' AND v_company_id IS NOT NULL THEN
        UPDATE profiles
