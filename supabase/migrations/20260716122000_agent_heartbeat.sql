-- Migration: Agent Heartbeat Timeout
-- This updates get_nearby_agents_dynamic to require that the agent's last_pulse is within the last 3 minutes for dispatch pickups.
...
BEGIN
    ...
    FROM (
        ...
        FROM profiles
        WHERE role = 'agent' 
          -- Heartbeat Check: Must be online AND have pulsed in the last 3 minutes
          -- OR be a physical hub (which doesn't require an active heartbeat)
          AND (
            (
                is_online = true 
                AND location->>'last_pulse' IS NOT NULL 
                AND (location->>'last_pulse')::timestamp >= NOW() - INTERVAL '3 minutes'
            )
            OR is_hub_active = true
          )
